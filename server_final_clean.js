...
app.get('/api/djs', (req, res) => {
    try {
        // Create a new array with just the DJs data
        const djsArray = [...djs];
        
        // Set proper content type and send only the array
        res.setHeader('Content-Type', 'application/json');
        res.send(JSON.stringify(djsArray));
    } catch (error) {
        console.error('Error sending DJs:', error);
...
app.listen(PORT, () => {
    ...
    console.log('Available DJs:', JSON.stringify(djs, null, 2));
});
